...
def _restore_context(data: dict[str, Any] | None) -> BusinessContext:
    ...
    for item in evidence_items: context.add_evidence(Evidence(id=str(item["id"]), source=str(item.get("source", "")), claim=str(item.get("claim", "")), value=item.get("value"), confidence=int(item.get("confidence", 0)), collected_at=str(item.get("collected_at", utc_now())), locator=item.get("locator"), metadata=dict(item.get("metadata", {}))))
    for item in data.get("relationships", []): context.link(Relationship(source_id=str(item["source_id"]), relation=str(item["relation"]), target_id=str(item["target_id"]), confidence=int(item.get("confidence", 100)), evidence_ids=list(item.get("evidence_ids", []))))
    return context
