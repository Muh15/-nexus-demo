...
def _restore_context(d):
    c=BusinessContext(); d=d or {}; entities=d.get("entities",[]); entities=list(entities.values()) if isinstance(entities,dict) else entities
    for x in entities:c.add_entity(Entity(str(x["id"]),str(x["type"]),str(x["name"]),dict(x.get("attributes",{}))))
    es=d.get("evidence",[]); es=list(es.values()) if isinstance(es,dict) else es
...
def _restore_goal_plan(d):
    ...
    g=d.get("goal") or {}; p=d.get("profile") or {}; goal=Goal(raw=str(g.get("raw","")),objective=str(g.get("objective","")),horizon=g.get("horizon"),target_value=g.get("target_value"),target_unit=g.get("target_unit"),constraints=tuple(g.get("constraints",[]))); profile=GoalProfile(key=str(p.get("key","general")),label=str(p.get("label","هدف أعمال عام")),evidence_domains=tuple(p.get("evidence_domains",())),action_posture=str(p.get("action_posture","measure_before_action"))); needs=[ResearchNeed(str(x["domain"]),str(x["reason"]),int(x.get("priority",50))) for x in d.get("research_needs",[])]; return GoalPlan(goal,profile,needs)
...
def _restore_action_plan(d):
    if not d:return None
    p=d.get("policy") or {}; policy=ActionPolicy(ActionRisk(str(p.get("risk","high"))),bool(p.get("requires_approval",True)),bool(p.get("allowed",False)),str(p.get("reason",""))); return ActionPlan(str(d.get("action_type","draft_email")),str(d.get("description","")),policy,dict(d.get("payload",{})))
...
class MissionOrchestrator:
    ...
    def create(self,*,tenant_id,goal,constraints=(),records=(),source="unknown"):
        constraints=list(constraints); records=list(records); m=MissionState(f"NXS-{uuid4().hex[:10].upper()}",tenant_id,goal,constraints); m.transition("observe","بدأ جمع الإشارات المرتبطة بالمهمة."); m.goal_plan=build_goal_plan(parse_goal(goal,constraints)); m.context=build_context(records,source=source); m.intelligence_graph=IntelligenceGraph.from_context(m.context); _,_,a=self._intelligence.prepare(tenant_id=tenant_id,goal_text=goal,constraints=constraints,records=records,source=source); m.impact_assessments=a; m.research_plan=build_research_plan(m.goal_plan,m.context,a); m.transition("understand","تم بناء السياق وخريطة الأدلة وتحديد فجوات المعلومات قبل القرار."); return m
    def research(self,m):
        ...
        m.transition("researched","انتهت دورة البحث ويمكن الآن تقييم كفاية الأدلة."); return m
    def decide(self,m):
