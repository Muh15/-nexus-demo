...
# ---------------------------------------------------------------------------
# Demo connectors. They already expose the interface we will later replace
# with real Gmail/ERP/CRM/file/API connectors.
# ---------------------------------------------------------------------------
...
def sample_signals() -> list[Signal]:
    return [
        Signal(
            ...
            title="Supplier ABC announced a price increase",
            value="+7% starting next cycle",
            impact="Raises cost on a high-spend category",
            confidence=97,
        ),
        Signal(
            id="sig-002",
            source=SourceType.ERP,
            title="ABC represents a large share of monthly spend",
            value="420,000 SAR / month",
            impact="Makes the price change financially material",
            confidence=95,
        ),
        Signal(
            id="sig-003",
            source=SourceType.MARKET,
            title="Market indicator moved in the opposite direction",
            value="-3% versus the last reference period",
            impact="Weakens the case for accepting a full +7% increase",
            confidence=86,
        ),
        Signal(
            id="sig-004",
            source=SourceType.CONTRACT,
            title="Contract permits renegotiation before the increase",
            value="43 days remaining",
            impact="Creates a safe intervention window",
            confidence=94,
        ),
        Signal(
            id="sig-005",
            source=SourceType.SUPPLIER,
            title="Alternative supplier has spare capacity",
            value="Up to 25% of current volume",
            impact="Provides negotiation leverage and a fallback",
            confidence=81,
        ),
    ]
...
def reason(goal: str, constraints: list[str], signals: list[Signal]) -> Decision:
    ...
    if cost_goal:
        return Decision(
            ...
            expected_impact="تقليل الزيادة المتوقعة من +7% إلى نطاق تفاوضي أقل، مع حماية الجودة واستمرارية التوريد.",
        )
    ...
    return Decision(
        ...
        confidence=82,
        rationale=[
            "تم تحليل الإشارات الأكثر ارتباطًا بالهدف.",
            "تمت موازنة الأثر مقابل القيود المحددة للمهمة.",
        ],
...
def health() -> dict[str, str]:
    return {"status": "ok", "service": "nexus-mvp", "version": "0.1.0"}
...
def create_mission(request: MissionRequest) -> Mission:
    ...
    log_event(audit, "observe", "تم جمع الإشارات من مصادر الأعمال التجريبية.")
...
def approve_mission(mission_id: str) -> Mission:
    ...
    mission.action = {
        "type": "email_draft",
        "status": "approved",
        "target": "ABC Industrial",
        "description": "تجهيز رسالة تفاوض أولية للمورد مع تسجيل الموافقة.",
    }
...
def execute_mission(mission_id: str) -> Mission:
    ...
    mission.action = {
        **(mission.action or {}),
        "status": "executed",
        "executed_at": utc_now(),
        "result": "تم إنشاء مسودة تفاوض آمنة وتجهيزها للإرسال.",
    }
...
def verify_mission(mission_id: str) -> Mission:
    ...
    mission.verification = {
        ...
        "verified_at": utc_now(),
        "checks": [
            "الإجراء مرتبط بالمهمة الأصلية",
            "المورد المستهدف صحيح",
            "لم يتم تجاوز أي عقد قائم",
            "تم حفظ سجل التدقيق",
        ],
    }
