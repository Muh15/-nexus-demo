...
def _isolated_store(tmp_path, name):
    ...
    original_runtime_repo = main.RUNTIME.mission_repository
    original_ingested = main.INGESTED_DATA
    repository = SQLiteMissionRepository(SQLiteMissionStore(tmp_path / name))
    main.MISSION_REPOSITORY = repository
    main.RUNTIME.mission_repository = repository
    main.INGESTED_DATA = {}
    return original_repo, original_runtime_repo, original_ingested


def _restore(original_repo, original_runtime_repo, original_ingested):
    main.MISSION_REPOSITORY = original_repo
    main.RUNTIME.mission_repository = original_runtime_repo
...
def test_api_mission_persists_across_cache_reset(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "api.sqlite3")
    try:
        response = client.post(
            "/api/missions",
            json={
        ...
        executed_at = executed_payload["audit"][-1]["timestamp"]
        ...
        assert verified_payload["action"]["execution_id"] == execution_id
        assert verified_payload["audit"][-1]["timestamp"]
        ...
        assert payload["action"]["executed_at"] if "executed_at" in payload["action"] else True
        assert executed_at
    finally:
        _restore(original_repo, original_runtime_repo, original_ingested)


def test_api_rejects_execute_before_approval(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "blocked.sqlite3")
    try:
...
def test_api_execution_is_idempotent(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "idempotent.sqlite3")
    try:
        mission_id = _create_mission()
        assert client.post(f"/api/missions/{mission_id}/approve").status_code == 200
...
def test_api_approval_and_verification_are_idempotent(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "idempotent_verify.sqlite3")
    try:
        mission_id = _create_mission()
...
def test_api_rejects_verify_before_execute(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "verify_blocked.sqlite3")
    try:
        mission_id = _create_mission()
...
def test_api_returns_404_for_unknown_mission(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "missing.sqlite3")
    try:
...
def test_api_rejects_invalid_file_ingestion(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "ingest.sqlite3")
    try:
        response = client.post(
            "/api/ingest/file",
...
def test_api_isolates_missions_between_tenants(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "tenant.sqlite3")
    try:
...
def test_api_isolates_ingested_records_between_tenants(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "ingest_tenant.sqlite3")
    try:
        alpha = {"X-Tenant-ID": "tenant-alpha"}
        beta = {"X-Tenant-ID": "tenant-beta"}
...
def test_api_rejects_invalid_tenant_id(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "tenant_validation.sqlite3")
    try:
...
def test_api_exposes_runtime_registry(tmp_path):
    original_repo, original_runtime_repo, original_ingested = _isolated_store(tmp_path, "runtime.sqlite3")
    try:
